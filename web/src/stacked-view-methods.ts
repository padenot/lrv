...
export class StackedViewMethods {
  ...
  private buildFileSection(file: DiffFile): HTMLElement {
    ...
    if (additions) statsEl.appendChild(el('span', { className: 'delta-add', text: `+${additions}` }));
    if (deletions) statsEl.appendChild(el('span', { className: 'delta-del', text: `-${deletions}` }));

    header.append(nameEl, statusEl, statsEl);
    section.appendChild(header);

    if (!file.hunks.length) {
      section.appendChild(el('div', { className: 'stacked-no-hunks', text: 'Binary or empty file.' }));
      return section;
    }
  ...
  private buildShowMoreRow(
    ...
    const btn = el('button', { className: 'stacked-show-more-btn',
  ...
  private showInlineCommentForm(
    ...
    const cancel = el('button', { className: 'stacked-comment-cancel btn-secondary', text: 'Cancel' });
  ...
  renderStackedComments() {
    ...
    comments.forEach((comment, idx) => {
      ...
      const del = el('button', { className: 'stacked-comment-del', text: '✕', attrs: { title: 'Delete' } });
