...
fn precompute_via_blobs(diffs: &[DiffResponse], working_dir: &str) -> ContentMaps {
    let n = diffs.len();
    let mut all_oids: Vec<String> = Vec::new();
    let mut oid_seen: HashSet<String> = HashSet::new();
    for diff in diffs {
        for file in &diff.files {
            for oid in [&file.old_blob, &file.new_blob].into_iter().flatten() {
    ...
    let content = match query.side.as_str() {
        ...
        "old" => {
            ...
            if let Some(cached) = {
                ...
                let content =
                    resolve_old_content(diff, &state.context.working_directory, &query.path);
                let mut cache = state.old_caches[commit_idx].lock().await;
...
async fn add_review_note(
    State(state): State<AppState>,
    Json(note): Json<ReviewNote>,
) -> StatusCode {
    if !note.is_valid() {
        return StatusCode::BAD_REQUEST;
    }
    let mut notes = state.review_notes.lock().await;
    notes.push(note);
    StatusCode::OK
}
