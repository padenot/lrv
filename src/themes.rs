...
fn convert(id: String, vsc: VsCodeTheme) -> UserTheme {
    ...
    let base = if vsc.theme_type.as_deref() == Some("light") { "vs" } else { "vs-dark" };
    ...
    for tc in vsc.token_colors {
        ...
        for scope in scopes {
            rules.push(MonacoRule { token: scope, foreground: foreground.clone(), font_style: font_style.clone() });
        }
    }
    ...
    UserTheme {
        ...
        name,
        accent_hex,
        data: MonacoThemeData { base: base.to_string(), inherit: true, rules, colors: vsc.colors },
    }
}
