...
async fn bench_precompute_firefox_series() {
    ...
    eprintln!("Enumerated {} commits in {:.1}ms", commit_ids.len(), t_enum.as_secs_f64() * 1000.0);
    ...
    let (old_maps, new_maps) =
        lrv::server::precompute_series_content(&diffs, FIREFOX_DIR).await;
    ...
    eprintln!("Total startup time: {:.1}ms", t_total.as_secs_f64() * 1000.0);
